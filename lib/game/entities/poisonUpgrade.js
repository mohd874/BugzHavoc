	...
	EntityPoisonUpgrade = EntityUpgradable.extend({
		...
		update: function(){
			...
			if(this.currentAnim === this.anims.inShop){
				...
				if(this.effectDuration.delta() >= 0){
					this.target.poison = 'empty'
					this.target.poisoned = false
					this.kill()
				}
			}
		},
		...
		applyEffect: function(entity){
			...
			if(this.currentAnim === this.anims.inShop && this.curLevel > 0){
				ig.log('this.currentAnim === this.anims.inShop: '+(this.currentAnim === this.anims.inShop))
				ig.log('this.curLevel: '+this.curLevel)
				ig.log('this.id: '+this.id)
				if(entity.poisoned){
					entity.poison.resetPoison()
				}else{
